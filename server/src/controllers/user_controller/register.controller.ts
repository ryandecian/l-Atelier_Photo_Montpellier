...
import InsertUserRepository from "../../repository/user_tbl/insertOneUser.repository";

/* Import des utils */
import { hashPasswordArgon_utils } from "../../utils/hashArgon.utils";

/* Import des Types : */
import getAllUser_controller from "./getAllUser.controller";
...
const register_controller = async (req: Request, res: Response) => {
        try {
            /* Logique métier 1 : Vérification si l'email existe */
                const dataUser: getAllUsers_type[] = await verifyEmailFalseRepository(req.body.email);
            ...
            /* Logique métier 3 : Insertion de l'utilisateur dans la DB */
                ...
                const insertUser: ResultSetHeader = await InsertUserRepository(req.body);
